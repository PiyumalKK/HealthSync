...
\fill[lightBg, rounded corners=6pt] (-2.8,-2.6) rectangle (10.8,2.4);
\draw[borderGray, rounded corners=6pt, line width=0.5pt] (-2.8,-2.6) rectangle (10.8,2.4);
\node[font=\small\bfseries\color{azureDarkBlue}] at (4,1.9) {Application CI/CD Pipeline};

% Backend row
\node[font=\scriptsize\bfseries\color{azureBlue}] at (-1.5, 0.7) {Backend};
\node[trigger] (dispatch) at (0.8,0.7) {Dispatch\\{\tiny select service}};
\node[stage] (build) at (3.6,0.7) {Docker\\Build};
\node[stage] (push) at (6.2,0.7) {Push ACR\\{\tiny SHA tag}};
\node[stage] (deploy) at (8.8,0.7) {az update\\{\tiny Container App}};
\draw[arrow] (dispatch) -- (build);
\draw[arrow] (build) -- (push);
\draw[arrow] (push) -- (deploy);
\node[font=\tiny\itshape, color=azureGray, anchor=south] at (8.8,1.3) {$\times$7 in parallel};

% Frontend row
\node[font=\scriptsize\bfseries\color{azureTeal}] at (-1.5,-1) {Frontend};
\node[stage, fill=azureTeal!8, draw=azureTeal!60] (fbuild) at (3.6,-1) {Vite Build\\{\tiny npm run build}};
\node[stage, fill=azureTeal!8, draw=azureTeal!60] (fupload) at (6.2,-1) {Upload to\\Blob \$web};
\node[stage, fill=azureTeal!8, draw=azureTeal!60] (fpurge) at (8.8,-1) {CDN Purge\\{\tiny Front Door}};

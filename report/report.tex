...
\begin{titlepage}
    ...
    \begin{tabular}{@{}l l}
        \textbf{Group Members} & : EG/2021/4735 --- Ranasinghe K.K.M.P\\
                                & \phantom{: }EG/2021/4598 --- Karunarathna C.K \\
                                & \phantom{: }EG/2021/4877 --- Wijesinghe S.A \\
                                & \phantom{: }EG/2021/4651 --- Madakaladeniya I.U \\[0.3cm]
        \textbf{Date}          & : 28th April 2026 \\
    \end{tabular}
...
\resizebox{0.95\textwidth}{!}{%
\begin{tikzpicture}[
    stage/.style={rectangle, rounded corners=6pt, draw=azureBlue!60, fill=azureBlue!8,
                  text width=3.2cm, minimum height=1.6cm, align=center, font=\small\bfseries,
                  drop shadow={opacity=0.08}},
    trigger/.style={rectangle, rounded corners=6pt, draw=azureGreen!60, fill=azureGreen!8,
                   text width=3.2cm, minimum height=1.6cm, align=center, font=\small\bfseries,
                   drop shadow={opacity=0.08}},
    sonarbox/.style={rectangle, rounded corners=6pt, draw=orange!60, fill=orange!8,
                  text width=3.2cm, minimum height=1.6cm, align=center, font=\small\bfseries,
                  drop shadow={opacity=0.08}},
    arrow/.style={-{Stealth[length=3mm]}, thick, color=azureGray!70},
]

% Large background rectangle
\fill[lightBg, rounded corners=10pt] (-3.5,-4.0) rectangle (19.5,3.6);
\draw[borderGray, rounded corners=10pt, line width=0.6pt] (-3.5,-4.0) rectangle (19.5,3.6);

\node[font=\large\bfseries\color{azureDarkBlue}] at (7.8,3.0) {Application CI/CD Pipeline};

% ── Backend row ──────────────────────────────────────────────
\node[font=\normalsize\bfseries\color{azureBlue}] at (-1.8, 0.8) {Backend};

\node[trigger]  (dispatch) at (1.2,  0.8) {Dispatch\\{\footnotesize select service}};
\node[sonarbox] (sonar)    at (4.8,  0.8) {Sonar\\{\footnotesize Code Scan}};
\node[stage]    (build)    at (8.4,  0.8) {Docker\\Build};
\node[stage]    (push)     at (12.0, 0.8) {Push ACR\\{\footnotesize SHA tag}};
\node[stage]    (deploy)   at (15.6, 0.8) {az update\\{\footnotesize Container App}};

\draw[arrow] (dispatch.east) -- (sonar.west);
\draw[arrow] (sonar.east)    -- (build.west);
\draw[arrow] (build.east)    -- (push.west);
\draw[arrow] (push.east)     -- (deploy.west);

\node[font=\footnotesize\itshape, color=azureGray, anchor=south] at (15.6,1.8) {$\times$7 in parallel};

% ── Frontend row ─────────────────────────────────────────────
\node[font=\normalsize\bfseries\color{azureTeal}] at (-1.8,-2.0) {Frontend};

\node[stage, fill=azureTeal!8, draw=azureTeal!60] (fbuild)  at (8.4, -2.0) {Vite Build\\{\footnotesize npm run build}};
\node[stage, fill=azureTeal!8, draw=azureTeal!60] (fupload) at (12.0,-2.0) {Upload to\\{\footnotesize Blob \$web}};
\node[stage, fill=azureTeal!8, draw=azureTeal!60] (fpurge)  at (15.6,-2.0) {CDN Purge\\{\footnotesize Front Door}};

% Sonar forks cleanly down then across to fbuild
\draw[arrow, color=azureTeal!70]
    (sonar.south) -- ++(0,-0.9) -| (fbuild.west);

\draw[arrow, color=azureTeal!70] (fbuild.east)  -- (fupload.west);
\draw[arrow, color=azureTeal!70] (fupload.east) -- (fpurge.west);

\end{tikzpicture}
}
\caption{Application pipeline: per-service Docker build, ACR push, Container App update.
Sonar code scan runs after dispatch for both Backend and Frontend. Frontend uploads to Blob Storage.}
...
\begin{enumerate}[noitemsep,topsep=2pt]
    ...
    \item \textbf{Code Quality Scanning:} SonarCloud analysis per microservice in CI/CD pipelines to detect bugs, code smells, and security vulnerabilities before deployment.
\end{enumerate}
